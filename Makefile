...
test-api:
	...
	docker run -d --network calc-test-api --env PYTHONPATH=/opt/calc --name apiserver2 --env FLASK_APP=app/api.py -p 5000:5000 -w /opt/calc calculator-app:latest flask run --host=0.0.0.0
	docker run --network calc-test-api --name api-tests --env PYTHONPATH=/opt/calc --env BASE_URL=http://apiserver2:5000/ -w /opt/calc calculator-app:latest pytest --junit-xml=results/api_result.xml -m api  || true
	docker cp api-tests:/opt/calc/results ./
	docker stop apiserver2 || true
	docker rm --force apiserver2 || true
...
test-e2e:
	...
	docker run -d --network calc-test-e2e --env PYTHONPATH=/opt/calc --name apiserver2 --env FLASK_APP=app/api.py -p 5000:5000 -w /opt/calc calculator-app:latest flask run --host=0.0.0.0
	docker run -d --network calc-test-e2e --name calc-web -p 80:83 calc-web
